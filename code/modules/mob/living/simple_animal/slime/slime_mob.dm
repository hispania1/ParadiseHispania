...
/mob/living/simple_animal/slime/attackby(obj/item/I, mob/living/user, params)
	...
	if(I.force >= 3)
		...
		if(prob(10 + force_effect))
			discipline_slime(user)
	if(istype(I, /obj/item/storage/bag/bio))//hispania changes start & end here , remueve comment
		...
		if(!hasOutput)
			if(!hasFound)
				...
				playsound(src, 'sound/effects/attackblob.ogg', 50, TRUE)
		return //*/ hispania changes start & end here ,remueve comment
	..()
///* hispania changes start & end here ,remueve comment
