/proc/add_note(target_ckey, notetext, timestamp, adminckey, logged = 1, checkrights = 1, show_after = TRUE, automated = FALSE, sanitise_html = TRUE) // Dont you EVER disable this last param unless you know what you're doing
	...
	if(logged)
		...
		GLOB.discord_manager.send2discord_simple(DISCORD_WEBHOOK_NOTES, "[usr.ckey] agregó una nota al jugador [target_ckey]: \n[notetext]")
		if(show_after)
...
/proc/remove_note(note_id)
	...
	GLOB.discord_manager.send2discord_simple(DISCORD_WEBHOOK_NOTES, "[usr.ckey] removió una nota hecha por [adminckey] al jugador [ckey]: \n[notetext]")
	show_note(ckey)
...
/proc/edit_note(note_id)
	...
	if(query_find_note_edit.NextRow())
		...
		GLOB.discord_manager.send2discord_simple(DISCORD_WEBHOOK_NOTES, "[usr.ckey] editó una nota hecha por [adminckey] al jugador [target_ckey]: \n[old_note] \n\nNueva nota: \n[safe_text]")
		show_note(target_ckey)
		qdel(query_update_note)
