/obj/structure/chair/noose //It's a "chair".
	...
	icon_state = "noose"
	icon = 'modular_hispania/icons/obj/miscellaneous.dmi'
	layer = FLY_LAYER
	flags = NODECONSTRUCT
	item_chair = null
	var/image/over
...
/obj/structure/chair/noose/user_buckle_mob(mob/living/carbon/human/M, mob/user)
	...
	if(M == user && buckle_mob(M))
		...
		playsound(user.loc, 'modular_hispania/sound/effects/noosed.ogg', 50, 1, -1)
...
/obj/structure/chair/noose/post_buckle_mob(mob/living/M)
	if(has_buckled_mobs())
		...
		M.pixel_y = M.get_standard_pixel_y_offset(M.layer)
...
/obj/structure/chair/noose/process()
	...
	for(var/mob/living/buckled_mob in buckled_mobs)
		if(pixel_x >= 0)
			...
			animate(buckled_mob, pixel_x = 3, time = 45, easing = ELASTIC_EASING)
		if(buckled_mob.mob_has_gravity())
			buckled_mob.AdjustLoseBreath(5 SECONDS)
			if(prob(30))
			...
			if(prob(20))
				...
				playsound(buckled_mob.loc, 'modular_hispania/sound/effects/noose_idle.ogg', 30, 1, -3)
