...
/obj/structure/femur_breaker
	...
	icon = 'modular_hispania/icons/obj/femur_breaker.dmi'
...
/obj/structure/femur_breaker/proc/drop_slat(mob/user)
	if(buckled_mobs.len)
		...
		if(!H)
			return
		if(H.stat == CONSCIOUS)
			playsound(src, 'modular_hispania/sound/machines/femur_breaker.ogg', 50, FALSE)
...
/obj/structure/femur_breaker/proc/attract_oldman()
	for(var/mob/living/simple_animal/hostile/oldman/M in world)
		if(!M)
			return
		var/mob/living/carbon/human/H = buckled_mobs[1]
		...
		spawn(2 SECONDS)
			...
			playsound(src, 'modular_hispania/sound/effects/oldman/sacrifice.ogg', 50, FALSE)
			...
			M.invisibility = 0
			var/old_alpha = src.alpha
			src.set_opacity(0)
			src.alpha = 125
			spawn(14 SECONDS)
				H.gib()
				spawn(2 SECONDS)
					for(var/turf/T in oview(3,M)) //Make this respect walls and such
						M.add_splatter_floor(T)
						new /obj/effect/decal/cleanable/blood/oil/sludge (T)
					M.icon_state = "idle"
					M.death()
					src.set_opacity(1)
					src.alpha = old_alpha
